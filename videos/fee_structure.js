...
test(video_title, async t => {
	...
	await f.add_slide({
		title: "Fee Structure",
		bullet_points: [ 'Fee Category', 'Fees Accounting' ],
			})

	await f.speak(`Fee Structure is where you select fee component and define formula based on which student fee will be calculated.`)

	await f.remove_slide()

	await f.speak(`Before creating a fee structure, we should create fee categories.`)
	...
	await f.speak(`Enter name for fee category.`)
	...
	await f.speak(`Now, let's create Accounts which fee will be booked as an income.`)

	await f.search("chart of ac", "Open Chart of Accounts")

	await f.speak(`Let's create income account for fees collection.`)
	...
	await f.fill_field("New Account Name", "Student Fee")

	await f.fill_field("Account Type", "Income Account")
	...
	await f.speak(`Same way, we can also add fee receivable account.`)
	...
	await f.fill_field("Fee Structure", "FS00010", "10")
