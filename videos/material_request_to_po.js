...
test('Material Request to Purchase Order', async t => {
	...
	await f.speak(`
		...
		In this video, we will learn how to initiate procurement process in E R P Next. To submit purchase request, create 
		a Material Request.`)
	...
	await f.speak(`You can filter based on Sales Order I D.`)

	await f.fill_field("search_term","41-1")
	...
	await f.speak(`
			Item details are fetched from Material Request. Update date by which this item is required.
			`)
...
//
// 	await f.fill_field("required_date","28-07-2017")
//
// 	await f.close_row()
	...
	await f.show_message('Go back to Material Request', 2);
	...
	await f.speak(`
			Once a Material Request is submitted, you can these transactions against it. Once a Purchase Order is created 
