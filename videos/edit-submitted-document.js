...
test(video_title, async t => {
	...
	await f.speak(`
		...
		E R P Next has many transactions and masters which can be submitted.

		Once a document is submitted, then values in all the fields are frozen, hence you cannot further edit it.
		
		To be able to edit any submitted document, you should first cancel, and then amend it.
		
		Let's check a submitted Purchase Order, and amend it.
		`)
	...
	await f.speak(`After submission, values in the fields will be frozen once again.
			...
			Being a system manager, you will be able to control cancel and amend permission for the users.
			`)
	...
	await f.speak(`You can see that Purchase User role, cancel and amend permission is not assigned.`)
