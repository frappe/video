...
module.exports = function (err) {
    ...
    switch (err.type) {
        ...
        case ErrCodes.INVALID_NETWORK_AUTHENTICATION_CREDENTIALS_FORMAT:
            m('(line %s): Network authentication credentials have an incorrect format. Use the following format ' + 'for the @auth directive: "@auth username:password".', err.line);
            break;
        ...
        case ErrCodes.MISPLACED_DIRECTIVE:
            m('(line %s): Misplaced directive. Directives (@fixture, @page, etc.) should be declared ' + 'in the global scope.', err.line);
            break;
        ...
        case ErrCodes.ACTION_FUNC_IS_NOT_A_LAST_ENTRY:
            m('(line %s): Action call is not the last statement in a step. Test steps ' + 'should not contain statements that follow an action call.', err.line);
            break;
        ...
        case ErrCodes.ASYNC_FUNC_CALL:
            m('(line %s): Async function call. Tests should not call asynchronous functions. ' + 'Use act.wait() instead.', err.line);
            break;
