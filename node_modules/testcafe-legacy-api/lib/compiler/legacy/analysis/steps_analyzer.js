...
StepsAnalyzer.prototype.run = function (astPath, filename, src) {
    ...
    if (!steps.length) this._err(this.ERR.FACILITY_IS_EMPTY, astPath);else this._analyzeSteps(name, steps);
};
...
StepsAnalyzer.prototype._analyzeSteps = function (facilityName, steps) {
    ...
    if (!this.isMixin) this.rawStepData[facilityName].testCasesDirectiveAst = this._tryExtractTestCasesDirectiveAst(steps);
    ...
    steps.forEach(function (step) {
        ...
        if (stepEntryName !== 'function') {
            ...
            if (stepEntryName === 'string' && analyzer._tryAnalyzeAsMixinInsertion(stepEntryName, stepAst, facilityName, step)) {
                return;
            }

            //NOTE: we have inIFrame call candidate
            if (stepEntryName === 'call' && stepAst[1][1] === 'inIFrame') analyzer._analyzeAsInIframeCall(stepAst, facilityName, step);

            //NOTE: it's not mixin or inIframe - just fail
            else analyzer._err(analyzer.ERR.STEP_IS_NOT_A_FUNCTION, stepAst);
...
StepsAnalyzer.prototype._tryAnalyzeAsMixinInsertion = function (stepEntryName, stepAst, facilityName, step) {
    ...
    if (match && match[1] === Common.MIXIN_INSERTION_POINT_DIRECTIVE_LVALUE) {
        if (this.isMixin) this._err(this.ERR.MIXIN_USED_IN_MIXIN, stepAst);else {
...
StepsAnalyzer.prototype._analyzeAsInIframeCall = function (stepAst, facilityName, step) {
    if (stepAst[2].length < 2) this._err(this.ERR.INIFRAME_FUNCTION_SHOULD_ACCEPT_TWO_ARGS, stepAst);else {
        var selectorArgAst = stepAst[2][0],
            stepBodyAst = stepAst[2][1];

        if (Ast.getEntryName(stepBodyAst) !== 'function') this._err(this.ERR.STEP_IS_NOT_A_FUNCTION, stepAst);else {
            this._analyzeStepContent(stepBodyAst);

            if (selectorArgAst[0].name !== 'function') {
                //NOTE: modify iFrame selector to a function
                stepAst[2][0] = ['function', null, [], [['return', selectorArgAst]]];
            }
