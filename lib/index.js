...
export var f = {
	...
	hover_element: async function(element, time) {
		for (let i = 0; i < time; i++) {
			await t.drag(element, 5, 0);
		}
		// await t.wait(time * 1000)
	...
	scroll_to_section: async function(section) {
		await f.scroll_to_element('.section-head:contains('+ section + '):visible');
	},
	...
	click_list_item: async function(text) {
		await t.click(Selector('.list-item a').withText(text));
	},
	...
	click_dropdown_item: async function(text) {
		const dropdown_item = Selector(text => $(`.dropdown-menu:visible li:contains(${text})`)[0])
		await t.click(dropdown_item(text));
	},
