...
export default {
	async fill_field(fieldname, value, keyword) {
		...
		if (fieldtype == "Link") {
			...
			// click link option
			await this.click_element('.link-field li', value);
		}
	...
	async scroll_to_section(section, time) {
		await this.scroll_to_element('.form-section', section, time);
	},
